    ...
    addMarker: function(marker) {
        ...
        if (marker.data.type == 0) {
          ...
          if(lastMarker != null) {
            ...
            var b = (marker.lat - lastMarker.position.A);
            if (b < 0) {
              b = b * -1;
            }

            var c = (marker.lng - lastMarker.position.F);
            if (c < 0) {
              c = c * -1;
            }
            ...
            //console.log("b: " + b + "|c: " + c + "|a: " + a + "|sin: " + sin + "|asin: " + asin + "|degree: " + degree);

            var virado = null;

            //se a nova imagem está no 3 quadrante
            if (marker.lat < lastMarker.position.lat() && marker.lng > lastMarker.position.lng()) {
              degree = 180 - degree;
              virado = 3;
            }
            //se a nova imagem está no 2 quadrante
            else if (marker.lat > lastMarker.position.lat() && marker.lng > lastMarker.position.lng()) {
              degree = 90 - degree;
              virado = 2;
            }
